...
var AlbumComponent = /** @class */ (function () {
    function AlbumComponent(servicesImagenes) {
    ...
    AlbumComponent.prototype.LlenarAlbum = function () {
        var _this = this;
        var url = this.path;
        var imagenesServidor = new Array();
        this.servicesImagenes.get(url).subscribe(function (retorno) {
            imagenesServidor = retorno;
            for (var recorre = 0; recorre < imagenesServidor.length; recorre++) {
                var recorreObjeto = imagenesServidor[recorre];
                _this.album.push({
                    id: recorreObjeto.id,
                    path: _environments_environment_prod__WEBPACK_IMPORTED_MODULE_1__["host"] + recorreObjeto.path,
                    widthImg: _this.widhtImg,
                    heigthImg: _this.heigthImg
                });
            }
    ...
    AlbumComponent.prototype.FilterAlbums = function (desde, hasta) {
        return this.album.filter(function (x) { return x.id >= desde && x.id <= hasta; });
    };
    AlbumComponent.prototype.Next = function (tipo) {
        var card = document.getElementById("album");
        var scroll = document.getElementById(this.idScroll);
        var viewPortScrolll = Number(scroll.getBoundingClientRect().width.toFixed(0));
        var item = Number(document.querySelector("#" + this.idScroll + " div").getBoundingClientRect().width.toFixed(0));
        var move = 0;
        var twoitems = item * 2;
...
var ImagenesService = /** @class */ (function () {
    ...
    ImagenesService.prototype.get = function (direccion) {
        return this.http.get(direccion, {
            responseType: "json"
        });
    };
    ImagenesService.prototype.getAll = function () {
        var direccion = src_environments_environment_prod__WEBPACK_IMPORTED_MODULE_3__["environment"].endpointImagenes;
        return this.http.get(direccion);
    };
...
var host = 'http://www.integritysolutions.com.ec/APIISC/';
var environment = {
    production: true,
...
//# sourceMappingURL=default~inicio-inicio-module~trabaja-nosotros-trabaja-nosotros-module-es5.js.map